...
class BestAlbum {
    ...
    fun solution(genres: Array<String>, plays: IntArray): IntArray = Array(genres.size) { Song(genres[it], it, plays[it]) }
            .groupBy { it.genre }.values
            .map {
                it.fold(0) { acc, song -> acc + song.play } to
                        it.sortedWith(Comparator { song1, song2 ->
