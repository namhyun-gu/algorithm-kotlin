...
fun solution14891(args: Array<String>) = with(Scanner(System.`in`)) {
    ...
    repeat(K) {
        ...
        rotated.forEach {
            wheels[it.first] = wheels[it.first].rotateWheel(it.second)
        }
    }

    val score = wheels.foldIndexed(0) { index, acc, s ->
